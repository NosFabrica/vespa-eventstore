...
./gradlew :store:test --tests "com.vitorpamplona.quartz.eventstore.store.NostrSemanticsStoreTest"
...
- Design docs in `docs/` (`scaling.md`, `multi-node-consistency.md`, `server-side-constraints.md`, `embedded-vespa-study.md`) record operator guidance and studied-but-rejected alternatives — check them before re-proposing one.
