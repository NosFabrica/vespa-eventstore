...
> **In this repo (vespa-eventstore):** `store/mapping/SearchExtractors` mirrors this surface
> with weighted fields (primary/secondary/body — authoritative table in README). At every
> Quartz pin bump, diff `references/searchable-kinds.md` (this skill's copy vs upstream's)
> to catch newly-searchable kinds and changed `indexableContent()` bodies, then update
> `SearchExtractors` and the README table. Our search-string extensions (`observer:`,
> `sort:rank`, `filter:rank:gte:N`, `include:spam`) ride the raw-string path this skill
