...
> **In this repo (vespa-eventstore):** the consumers of this model are `store/trust/` —
> `TrustProjection` (projects 10040/30382 into per-pubkey `reputation` parent documents for
> ranking), `TrustReconciler`, and `sweepOrphanScores()` (drops 30382s signed by services no
> readable 10040 names — the "legitimate storage policy" this skill mentions under
> Authorization). Multi-provider combination is unprescribed upstream; our policy lives in
