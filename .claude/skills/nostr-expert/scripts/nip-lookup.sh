...
#
# vespa-eventstore has no local quartz/ checkout (Quartz is a JitPack dependency),
# so set QUARTZ_PATH to a clone of amethyst, e.g.:
#   QUARTZ_PATH=/tmp/amethyst/quartz/src/commonMain/kotlin/com/vitorpamplona/quartz $0 50
...
if [ ! -d "$QUARTZ_PATH" ]; then
    echo "error: Quartz sources not found at '$QUARTZ_PATH'." >&2
    echo "Clone amethyst and set QUARTZ_PATH (see the header of this script)." >&2
    exit 1
fi
